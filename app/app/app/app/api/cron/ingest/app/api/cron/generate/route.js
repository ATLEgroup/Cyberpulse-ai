...
export async function GET(req) {
  ...
  if (!sources || sources.length === 0) {
    return Response.json({ success: true, generated: 0, message: "No unprocessed sources" });
  }
  ...
  for (const source of sources) {
    try {
      ...
      const { error: insertError } = await supabase.from("articles").insert({
        ...
        severity: articleData.severity || null,
        cve_ids: articleData.cve_ids || [],
      });
      ...
      if (err.code === "NO_CREDIT") {
        errors.push("NO_CREDIT: Anthropic account has no API credit.");
        break;
      }
