...
export default async function ArticlePage({ params: paramsPromise }) {
  ...
  return (
    <>
      <header className="hdr"><div className="inner"><a href="/" className="logo">CYBER<span>PULSE</span> AI</a><span className="tagline">AI-POWERED THREAT INTELLIGENCE</span></div></header>
      <div className="art">
        ...
        {article.source_urls&&article.source_urls.length>0&&(
          <div className="sources"><h4>Sources</h4><ul>{article.source_urls.map((u)=><li key={u}><a href={u} target="_blank" rel="noopener noreferrer">{u}</a></li>)}</ul></div>
        )}
      </div>
